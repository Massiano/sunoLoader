...
def parse_suno_content(target_url):
    ...
    try:
        html_response = requests.get(resolved_url, headers=headers, timeout=10)
        html_metadata = parse_suno_song_html(html_response.text)
    except Exception:
        html_metadata = {}

    result = fetch_and_parse_rsc(resolved_url)
    if not result.get("is_rsc"): return {"resolved_url": resolved_url, "type": "unknown", "html_metadata": html_metadata, "raw": result}
